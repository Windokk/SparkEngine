...
void SceneLoader::Load2() {
	...
	for (int obj_id = 0; obj_id < parser.objects.size();obj_id++) {
		for (int a = 0; a < parser.objects[obj_id].components.size(); a++) {
			if (std::holds_alternative<ModelComponent>(parser.objects[obj_id].components[a])) {
				if (std::get<ModelComponent>(parser.objects[obj_id].components[a]).instancing != 1) {
					// The number of asteroids to be created
					const unsigned int number = std::get<ModelComponent>(parser.objects[obj_id].components[a]).instancing;
					
					const char* path = (std::get<ModelComponent>(parser.objects[obj_id].components[a]).model_path).c_str();
					...
					models.push_back(Model(path, obj_id, number, instanceMatrix)); 
				}
			}
		} 
	}
